...
if (!class_exists('K2ScatteredGalleryBaseHelper')) {
    /**
     * BaseHelper for com_content only.
     */
    abstract class K2ScatteredGalleryBaseHelper
    {
        /**
        ...
        public static function getK2Images($item, $params, $prefix = 'imgcfg')
        {
            ...
            if (!isset(self::$image_cache[$hash][$item->id])) {
                ...
                $images_path = array();
                $priority = preg_split('/[\s|,|;]/', $params->get($prefix . '_order', 'k2_image, image_intro, inline_introtext, external'), -1, PREG_SPLIT_NO_EMPTY);
                if (count($priority) > 0) {
                ...
                if (count($mark) > 0) {
                    ...
                    foreach ($mark as $type => $true) {
                        ...
                        switch ($type) {
                            ...
                            case 'external':
                                $exf = $params->get($prefix . '_external_url', 'images/k2/{id}/');
