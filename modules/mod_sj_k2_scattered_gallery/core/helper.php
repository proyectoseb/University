...
abstract class K2ScatteredGalleryHelper extends K2ScatteredGalleryBaseHelper
{
    public static function getList(&$params)
    {
        jimport('joomla.filesystem.file');
        $mainframe = JFactory::getApplication();
        $limit = $params->get('itemCount', 5) * 3;
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                $vote = new K2ModelItem();
                $item->votingPercentage = $vote->getVotesPercentage($item->id);
                $item->numOfvotes = $vote->getVotesNum($item->id);
    ...
    public static function  Convertday($date)
    {
        if ($date >= 11 && $date <= 13) {
            return $date . "th";
        }
